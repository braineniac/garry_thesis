...
In this experiment we compare our Simple Kalman Filter(SKF) to the robot\_localization package's Extended Kalman Filter(EKF). We used the same system input and experiment parameters as described in Section \ref{sec:adapt} for direct comparability.
...
\end{tikzpicture}
\end{flushleft}
\caption{EKF and SKF(sig) state estimation}\label{fig:ekfsig}
\end{figure}
