...
We decided to run the robot in an octagon pattern with a side length of aproximately 66cm-s. We chose this shape because of relative ease to drive the robot straight then turn and its similarity to a circle. By driving the robot in a loop through this octagon path, we can observe the drift in its localization.

As an input we used the Gamepad as an approximate velocity $\dot{x}$ in the robots relative x  direction, the accelerometer's relative x direction as $\ddot{x}$ and the gyroscope's angular z velocity as a $\dot{\psi}$. We estimated the covariance of the Gampad's velocity input to the accelerometer's to $\frac{1}{10}$.
...
Such a drift is a common issue in accelerometers and can be alleviated by relying on other sensors to close the loop and reset the sensor or using a sensor with a better or at least known in run bias stability.
