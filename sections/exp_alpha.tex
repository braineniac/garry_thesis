...
\begin{figure}[H]
 \begin{flushleft}
  ...
  \end{tikzpicture}
 \end{flushleft}
 \caption{Alpha tuning reference experiment with less stops}\label{fig:alphasingle}
\end{figure}
...
\begin{figure}[H]
 \begin{flushleft}
  ...
  \end{tikzpicture}
 \end{flushleft}
 \caption{Alpha tuning reference experiment with 10 meter distance}\label{fig:alphamulti}
\end{figure}
...
We run the same experiments as in the last section to see if the added inital IMU measurements impact our estimations.
...
We repeat the experiments similarly as in the last section with the Extended Kalman Filter, but we use $u_k^\eta$ and $y_k^\chi$ and the $\beta$ set from the next section for a more practical comparison.
