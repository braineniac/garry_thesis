...
We set $\mu_v=6$ since its the closest one to 5 meters. We can also see that higher values effect the state estimation slightly less than lower ones.
...
Next we tune our $\mu_{\dot{\Psi}}$ parameter with an experiment where we turn 5 times without jerking the controller to achieve a longer constant turn speed period, very similarly as in \ref{subsec:tunemuv}.
...
We set $\mu_{\dot{\Psi}}=0.147$, since the closest one to 0 or $2 \pi$. We can see that this parameter is very sensitive to small changes.
