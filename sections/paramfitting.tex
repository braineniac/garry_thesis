...
We have a few parameters that we need to set in our Kalman filter model. In the next subsections we will tune $\mu_v$ and $\mu_{\dot{\Psi}}$ to an appropriate value with the help of a few experiments. We assume that the rest of the parameters were already set.
...
\begin{figure}[H]
 \begin{flushleft}
  ...
  \end{tikzpicture}
\end{flushleft}
 \caption{Estimate of the x state with different $\mu_v$-s after driving the robot 5 meters in a stright line}\label{fig:muv}
\end{figure}

We set $\mu_v=6m$ since its the closest one to 5 meters. We can also see that higher values effect the state estimation slightly less than lower ones.
...
\begin{figure}[H]
 \begin{flushleft}
  ...
  \end{tikzpicture}
 \end{flushleft}
 \caption{Effects on the x state estimation when jerking the controller after driving the robot 5meters in a straight line}\label{fig:testmicrov}
\end{figure}
...
Next we tune our $\mu_{\dot{\Psi}}$ parameter with an experiment where we turn 5 times 360 degrees without jerking the controller to achieve a longer constant turn speed period, very similarly as in \ref{subsec:tunemuv}.
...
\begin{figure}[H]
  \begin{tikzpicture}
   ...
   \end{axis}
  \end{tikzpicture}
 \caption{Estimate of the $\Psi$ state with different $\mu_{\dot{\Psi}}$-s after turning 5 times 360 degrees}\label{fig:mupsi}
\end{figure}

We set $\mu_{\dot{\Psi}}=0.147[rad]$, since the closest one to 0 or $2 \pi$. We can see that this parameter is very sensitive to small changes.

\subsection{Testing $\mu_{\dot{\Psi}}$}\label{sebsec:testmudpsi}

Next we test our set $\mu_{\dot{\Psi}}$ in an experiment where we turn 5 times 360 degrees as well, but jerk the controller and stop frequently.
...
\begin{figure}[H]
 \begin{tikzpicture}
  ...
  \end{axis}
 \end{tikzpicture}
 \caption{Effects on the $\Psi$ state estimation when jerking the controller after turing 5 times 360 degrees}\label{fig:testmupsi}
\end{figure}

As we can see from Figure \ref{fig:testmupsi}, our model is not very accurate in describing the rotation of the robot. We are off about about 2 turns. We will reflect this in the process covariance $\textbf{Q}_k$ and rely more on the gyroscope.
