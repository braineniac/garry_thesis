...
and $M_1, M_2 \in \Re$ are constants for scaling the peak, $\textbf{q}_k^{\vartheta_k}, \textbf{q}_k^{\eta_k}$ are the queues corresponding to the elements of the input vector $\Delta \textbf{u}_k$.
...
We then run the comparison between our Kalman Filter with and without the adaptive feature and look at our changes in our v state and the process noise covariance matrix $\textbf{Q}_k$.
...
The $\textbf{Q}^\vartheta_k$ plot in Figure \ref{fig:sim_Q0} resembles our mirrored sigmoid function from Figure \ref{fig:sigwin}. In this case the window was set to a length of 10.
