...
\chapter{Kalman filter}\label{cha:customkalman}
...
\section{Kalman filter theory}\label{sec:kalmanfilter}
...
Our system input is a joystick controller, which we used to move our robot forward and backward by setting the voltage on the motors, which generate a force or torque.
...
To construct our dynamic coefficient matrix $ \boldsymbol{\Phi_k} $ and input coupling matrix $ \boldsymbol{\Gamma_k} $, we used the following equations based on the Taylor series expansion at $f(t_k+\Delta t)$:

\begin{align*}
    f(t_k+\Delta t) = f(t_k) + \frac{f'(t_k)}{1!} \Delta t + \frac{f''(t_k)}{2!} (\Delta t)^2 + ... + \frac{f^{n}(t_k)}{n!} (\Delta t)^n
\end{align*}

as:

\begin{align*}
    x_{k+1} &\approx x_{k} + v_k cos(\Phi_k) \Delta t + \frac{1}{2} a_k cos(\Phi_k) (\Delta t)^2\\
    y_{k+1} &\approx y_{k} + v_k sin(\Phi_k) \Delta t + \frac{1}{2} a_k sin(\Phi_k)(\Delta t)^2\\
    v_{k+1} &\approx v_{k} + a_k \Delta t\\
    \Psi_{k+1} &\approx \Psi_{k} + \dot{\Psi}_{k} \Delta t + \frac{1}{2} \ddot{\Psi}_k (\Delta t)^2\\
    \dot{\Psi}_{k+1} &\approx \dot{\Psi}_{k} + \ddot{\Psi}_{k} \Delta t\\
\end{align*}

and linear drag based on Newtonian mechanics:
...
\section{Kalman filter with adaptive process noise}\label{sec:kfapn}
...
We calculate the $w_k$ weights of a moving weighted window of length $N$ with $ \textbf{x}_k = \frac{k}{N}$ as:
\begin{align*}
...
\begin{figure}[H]
 \begin{center}
  \begin{tikzpicture}[
    declare function={sig(\x) = 1 / (1 + exp(7*(\x - 0.5)));}
   ]
   \begin{axis}[
  ...
  \end{tikzpicture}
\end{center}
 \caption{Moving Mirrored Sigmoid Weighted Window of $N=10$ and $\alpha=7$}\label{fig:sigwin}
\end{figure}
...
Next we we create the differential of the v state to get the a state and add some noise to be used as a system output.
...
\begin{flushleft}
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot+[color=red, mark=none,line width=1pt,mark size=1pt] table {plots/line_sim0_x2.csv};
      \addlegendentry{v state}
      \addplot+[color=blue, mark=none,line width=1pt,mark size=1pt] table {plots/line_sim0_u0.csv};
      \addlegendentry{$ \textbf{u}^\vartheta_k$}
    \end{axis}
  \end{tikzpicture}
\end{flushleft}
\caption{Simulation of the system input $ \textbf{u}^\vartheta_k$ and the desired v state of driving the robot in a straight line}\label{fig:vsimin}
\end{figure}
...
\begin{flushleft}
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot+[color=red, dashed, mark=none,line width=1pt,mark size=1pt] table {plots/line_sim0_y0.csv};
      \addlegendentry{$ \textbf{y}^\zeta_k$}
      \addplot+[color=blue, mark=none,line width=1pt,mark size=1pt] table {plots/line_sim0_x3.csv};
      \addlegendentry{a state}

    \end{axis}
  \end{tikzpicture}
\end{flushleft}
\caption{Simulation of the system output $ \textbf{y}^\zeta_k$ and the desired a state of driving the robot in a straight line}\label{fig:asim}
\end{figure}

\vspace{0.5cm}

We use the output from Figure \ref{fig:asim} as an output of our system. If we set $\textbf{Q}_k$ to a value close to zero, the v state estimation relies on the system model completely. If we set it high, it relies almost completely on the system output. These two extremes are illustrated in Figure \ref{fig:sysextreme}.
...
\begin{figure}[H]
 ...
 \end{subfigure}
 \caption{Simulation of the effects on the v state by setting $\textbf{Q}_k$ to extreme values}\label{fig:sysextreme}
\end{figure}

\vspace{0.5cm}

Next we set $\textbf{Q}_k$ somewhere in between with $\varrho_{\vartheta\zeta}=0.05$, which we used in the rest of the thesis and compare our Kalman filter with and without the adaptive process noise feature with the v state simulation in Figure \ref{fig:kalmansim}.
...
\begin{figure}[H]
 ...
 \begin{subfigure}[b]{0.5\textwidth}
  ...
  \caption{Falling edge}
 \end{subfigure}
 \caption{Comparison of the desired simulated v state and estimated v state by the Kalman filter and Kalman filter with adaptive process noise during rising and falling edge. The KF(APN) follows the measurement more closely before relying on the model again.}\label{fig:kalmansim}
\end{figure}
