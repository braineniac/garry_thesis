...
Our system input is a joystick controller, which we use to move our robot forward and backward by setting the voltage on the motors, which generate a force or torque.
...
\begin{flushleft}
  ...
  \end{tikzpicture}
\end{flushleft}
\caption{Simulation of the system input $ \textbf{u}^\vartheta_k$ and the desired v state of driving the robot in a stright line}\label{fig:vsimin}
\end{figure}
...
\begin{figure}[H]
 ...
 \begin{subfigure}[b]{0.5\textwidth}
  ...
  \caption{Falling edge}
 \end{subfigure}
 \caption{Comparison of the desired simulated v state and estimated v state by the Kalman filter and Kalman filter with adaptive process noise during rising and falling edge. The KF(APN) follows the measurement more closely before to relies on the model again.}\label{fig:kalmansim}
\end{figure}
